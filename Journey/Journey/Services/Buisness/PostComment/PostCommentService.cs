    ...
    public class PostCommentService : IPostCommentService
    {
        ...
        public async Task<Comment> AddCommentAsync(string comment, string post)
        {
            try
            {
                ...
                Comment commentDTo = await _postDataService.AddCommentAsync(comment, post);
                if (commentDTo != null)
        ...
        public async Task<List<Comment>> GetCommentsAsync(string post, bool sync = false)
        {
            try
            {
                List<Comment> commentList = await _postDataService.GetCommentsAsync(post, 0, PostPageSize, sync);
                return commentList;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> DeleteCommentAsync(string comment, string post)
        {
            try
            {
                bool deleted = await _postDataService.DeleteCommentAsync(comment, post);
                return deleted;
            }
            catch (Exception ex)
            {
